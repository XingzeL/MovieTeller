...
const content = {
  zh: {
    ...
    // Plans footer
    plansFooter: '所有套餐均包含：8 种语言支持 · 自动生成场景学习卡 · 高质量 TTS 解说 · 学习卡永久保留',

    // Addons
    addonsTitle: '额外付费 · 解说包',
    addonsSubtitle: '独立计费，不占用月度额度，可按需灵活补充',
    buyNow: '立即购买',

    // Footer
    tipsTitle: '温馨提示',
    tip1: '生成的视频文件在您下载一次后会按策略自动清理，学习卡长期保留。',
    tip2: '额度按实际处理时长扣除，失败任务不扣除额度。',
    ...
    // Alerts
    subscribeThanks: (planName: string) => `感谢选择 ${planName} 套餐！\n订阅功能即将上线，敬请期待。`,
    addonThanks: (addonName: string) => `感谢购买 ${addonName}！\n解说包购买功能即将上线。`,
  ...
  en: {
    ...
    plansFooter: 'All plans include: 8 language support · Auto scene study cards · High-quality TTS narration · Permanent study card retention',

    addonsTitle: 'Add-on Narration Packs',
    addonsSubtitle: 'Billed separately, does not consume monthly quota. Flexible top-ups as needed.',
    buyNow: 'Buy now',
    ...
    tip2: 'Quota is deducted based on actual processing time. Failed jobs do not consume quota.',
    ...
    subscribeThanks: (planName: string) => `Thank you for choosing the ${planName} plan!\nSubscription checkout will be available soon.`,
    addonThanks: (addonName: string) => `Thank you for purchasing ${addonName}!\nAdd-on purchase will be available soon.`,
...
export function PricingPage() {
  ...
  const addons: Addon[] = [
    {
      ...
      note: lang === 'zh' ? '不占月度额度' : 'Does not consume monthly quota',
    },
    {
      id: 'm',
  ...
  const handleSubscribe = (planId: string) => {
    if (planId === 'free') {
      alert(t.freeStartMessage)
      navigate('/create')
      return
    }
    const planName = monthlyPlans.find((p) => p.id === planId)?.name || planId
    alert(t.subscribeThanks(planName))
  }

  const handleBuyAddon = (addon: Addon) => {
    alert(t.addonThanks(addon.name))
  }
