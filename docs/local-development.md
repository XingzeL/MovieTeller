...
1. **最近任务**：列表来自 `GET /api/jobs`，点击可带 `?jobId=` 打开详情。
2. **上传创建 Job**：`POST /api/jobs`（MP4 等，见 API 文档）。
3. **任务面板**：状态、进度条、页内增量日志、取消、产物下载（成片 + 学习卡片）。
4. 选项：TTS 开关、CEFR、视频语言（ASR）、TTS 语言（含越南语 `vi` 等）。润色与字幕上下文**默认开启**（无勾选框，与完整管线一致）。
