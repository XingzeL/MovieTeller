...
def probe_duration_sec(video_path: str, *, ffprobe_bin: str) -> float:
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"ffprobe failed ({proc.returncode}): {proc.stderr.strip() or proc.stdout.strip()}"
        )
...
def segment_duration_sec(
    ...
    *,
    ...
    """
    Duration of the narrated segment in seconds.
    If both start and end are None, uses full file duration (short-clip mode).
    """
    if (start_sec is None) ^ (end_sec is None):
        raise ValueError("start_sec and end_sec must both be set or both be None")
    if start_sec is None and end_sec is None:
        return probe_duration_sec(video_path, ffprobe_bin=ffprobe_bin)
