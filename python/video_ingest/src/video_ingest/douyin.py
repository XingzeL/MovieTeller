...
def parse_douyin_video(url: str) -> dict:
    """Normalize Douyin parse output for MovieTeller router."""
    raw = DouyinParser().parse(url)
    return {
        "id": raw.get("id"),
        "title": raw.get("title"),
        "thumbnail": raw.get("thumbnail"),
        "duration": raw.get("duration"),
        "platform": raw.get("platform") or "douyin",
        "uploader": raw.get("uploader"),
    }


def download_douyin_video(url: str, output_dir: str) -> dict:
    """Download Douyin video into output_dir/source.mp4."""
    import os
    from pathlib import Path
    ...
    dest = Path(output_dir) / f"source{src.suffix or '.mp4'}"
    ...
    return {
        "path": str(dest),
        "filename": dest.name,
        "originalname": result.get("filename") or dest.name,
        "mimetype": "video/mp4",
        "size": size,
