...
from movie_pipeline.study_cards_html import export_study_cards_html, format_media_timestamp
...
def test_export_study_cards_html_external_assets(tmp_path: Path) -> None:
    pool = tmp_path / "pool2"
    img_dir = pool / "images"
    img_dir.mkdir(parents=True)
